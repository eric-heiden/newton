...
- Add `SolverSPH`, an experimental Warp-accelerated particle-fluid solver, and add ViewerGL screen-space fluid rendering with translucent smoothed water surfaces, tunable blur radius, tropical optical-depth color absorption, stronger env-map reflection/refraction and env transmission, scene-depth floor caustics, depth cueing, ripple/caustic distortion, and screen-space foam for particle fluids. Add a dam-break example plus an interactive tank example with a larger/deeper SPH basin, displaced-volume buoyant rigid boxes, initial particle carve-outs around those boxes, picking forces, SPH-to-box coupling with clamped splash torque feedback, Flex-style particle friction, collision margin, sleep threshold, velocity/acceleration limiting, neighbor-count dissipation, a larger 40k-particle default grid, render/diffuse update decimation, and live GUI sliders for water shader and interaction settings.
- Add primitive, mesh, and heightfield shape collision to `SolverSPH` fluid and diffuse particles for model shapes marked with particle collision, with optional dynamic rigid-body force feedback from main SPH particles plus Flex-style shape collision distance/margin, shape restitution, shape friction, and adhesion for damped sliding/wetting near contacted surfaces.
...
- Add `fluid_sph_wave_pool` example: a kinematic paddle drives waves across an SPH pool that break on a sloped beach and shed whitewater foam
...
- Improve ViewerGL water defaults and reflections with solver-owned smoothed render positions, solver-owned three-axis anisotropy, Flex-style anisotropy min/max clamps, 3D view-aware anisotropic fluid splats with ray-cast ellipsoid depth/thickness, depth-edge-aware screen-space surface and thickness smoothing, thickness-aware surface normals, per-channel depth-aware refraction and refracted-thickness compositing to reduce object-edge halos, valid water-surface depth writes, scene-shadow receiving water surfaces, 2048px light-space water-volume shadows with softened floor coverage, world-locked blue floor caustics, scene-depth contact foam, stronger screen-space object reflection, thickness/depth-limited environment and screen reflection, luminance-limited environment reflection/refraction, Flex-style sky/ground reflection tinting, stable back-to-front diffuse particle sorting, softened rounded-depth opaque and front/behind water diffuse-particle splits, deterministic diffuse-particle breakup, optional PBF-style density projection, XSPH velocity smoothing, Flex-style cohesion/surface-tension/vorticity/solid-pressure/free-surface-drag/buoyancy terms, and Flex-style diffuse foam/spray/bubble particles with outward emission velocities, randomized lifetimes, neighbor-density-driven foam radius/opacity, lifetime sprite expansion, light-space shadow attenuation, light scattering controls, and immediate bounds clamping for `SolverSPH`; headless GL examples now honor `--num-frames`; adjust `viewer.fluid_*`, `viewer.fluid_diffuse_*`, `SolverSPH.render_*`, `RendererGL.fluid_shadow_size`, or fluid example flags to restore previous values.
- Replace ViewerGL water-surface screen-space reflection with view-dependent environment-map reflection enabled by default using the `newton_envmap.jpg` probe and a screenshot-calibrated default `fluid_env_map_strength` of `1.02`, removing the camera-following reflection streaks on the surface; make submerged water cast a transparent blue floor shadow whose depth scales with fluid density rather than being gated out by deep or obliquely lit columns. Adjust `viewer.fluid_env_map_strength` / `--fluid-env-map-strength` to tune reflectivity and `viewer.fluid_surface_shadow_strength` / `--fluid-surface-shadow-strength` to tune the shadow.
...
- Overhaul ViewerGL screen-space water compositing into a Flex-style pipeline: one-sided finite-difference surface normals, Gaussian-range bilateral depth smoothing, per-channel Beer-Lambert absorption toward `fluid_deep_color`, Schlick-Fresnel blending of refraction against environment reflection plus single-tap screen-space scene reflection, wrap-diffuse sun lighting, tight specular highlights, and shadow-receiving water; defaults change for `fluid_smoothing_depth_edge_falloff` (`1.39` to `5.5`) and `fluid_surface_shadow_strength` (`0.03` to `0.35`). The interactive SPH tank example now derives box masses from densities (`--box-density-fractions`), measures the local water surface from the particles, and applies displaced-volume buoyancy with righting torque, quadratic drag, and tank-floor contact so light boxes float at density-proportional drafts while dense boxes sink; `--box-mass`, `--box-density`, `--suspension-stiffness`, `--suspension-damping`, and `--box-fluid-drag` are replaced by `--buoyancy-scale`, `--box-linear-drag`, `--box-quadratic-drag`, and box floor/wall contact parameters. Fluid render data is now packed on the GPU (one vertex-buffer upload instead of four device-to-host copies per frame), and the SPH examples lower the default PBF artificial pressure so the fluid rests at its fill level instead of inflating above it.
- Recast the ViewerGL underwater shadow as the water's own Beer-Lambert color filter evaluated from the light-space depth/thickness maps, so the floor shadow matches the rendered water's color and opacity (the previous bounds-box heuristics produced a detached gray rectangle). Make the interactive SPH tank water livelier (lower viscosity/XSPH/dissipation defaults) with ~56k particles at 3 substeps and 3 PBF iterations, capture the SPH substeps in a CUDA graph by default (re-captured automatically when GUI sliders change; pass `--no-capture-graph` to disable), and advance the diffuse-foam spawn seed on the device so foam keeps spawning inside captured graphs.
...
- Fix the interactive SPH tank example to apply fluid-shape feedback to the rigid integration step and add buoyancy torque so floating boxes can rotate under water and picking interaction.
- Fix ViewerGL diffuse foam/spray particles never rendering: the diffuse fragment shader reconstructed scene depth with a mismatched matrix convention, so every sprite was discarded as occluded; splash spray and whitewater foam are now visible. `SolverSPH` diffuse spawning now uses a trapped-air/wave-crest potential (separating relative motion weighted by velocity-surface alignment) instead of raw kinetic energy, so foam forms at breaking crests, splashes, and beach run-up fronts rather than inside fast bulk flow or on the back face of waves, and foam sprites scale non-uniformly Flex-style: fast spray stretches along its velocity while settled surface foam flattens into pancakes along the screen projection of world-up. Diffuse buoyancy/drag defaults follow Flex (buoyancy ~1) so foam rises to the surface instead of being entrained underwater. Underwater shadows and long view paths through water keep an in-scattered ambient floor instead of fading to black at pool edges
