...
- Add a render-particle limit to `SolverXPBD.update_render_particles()` for scalable fluid-surface previews.
- Add `fluid_xpbd_cup` example: a single rigid cup of XPBD water that you grab and swing around with the mouse -- the minimal "fluid in a moving container" scene (no arm/IK) for profiling fluid-in-container performance.
- Add `SolverXPBD.reorder_particles()` to sort free fluid particles into Morton (spatial) order, keeping the position-based-fluid density solve's hash-grid neighbor reads cache-coherent as the fluid churns; it is a pure relabel, CUDA-graph-capturable, and a no-op unless all active particles are fluid.
- Add `fluid_max_neighbors` to `SolverXPBD`: an optional cap on the neighbors each fluid particle processes in the density solve, bounding the cost of momentary over-compressed clumps (which otherwise stall a whole GPU warp). Defaults to `0` (unlimited); set it above the bulk neighbor count so the rest state is unaffected.
- Add `body_max_velocity` and `body_max_angular_velocity` to `SolverXPBD`: optional per-substep clamps on dynamic-body linear/angular speed that stop a body driven into deep penetration from diverging to NaN. Both default to `0` (disabled).
- Add `fluid_xpbd_multiworld_cup` example with two overlapping, pickable XPBD fluid cups in separate worlds that remain isolated by grouped hash-grid neighbor queries.
- Add `fluid_xpbd_multi_fluid_tank` example showing multiple XPBD fluid density/material groups interacting with floating rigid bodies in one tank.
- Add `newton.utils.download_git_folder()` as the public helper for downloading a folder from an external Git repository.
- Add Flex-style screen-space fluid rendering to `ViewerGL` via `Viewer.log_fluid()` and `Viewer.log_fluid_diffuse()`: anisotropic quadric ellipsoid splatting, adaptive separable bilateral depth smoothing, a composite pass with screen-space refraction/reflection, Fresnel, shadow-mapped wrap lighting and tight speculars, fluid splats in the scene shadow map, and velocity-stretched diffuse foam billboards. Ported from the NVIDIA Flex demo renderer.
...
- Add position-based fluid (PBF) support to `SolverXPBD`: particles flagged with the new `ParticleFlags.FLUID` generate density constraints solved alongside all other XPBD constraints, with controls for rest spacing, cohesion, viscosity, and vorticity confinement, plus `SolverXPBD.update_render_particles()` for Flex-style smoothed/anisotropic fluid surface rendering
- Generate particle soft contacts against mesh shapes that carry a texture SDF (built via `Mesh.build_sdf()`) by sampling the SDF (one contact per particle) instead of per-triangle mesh queries, so large particle/fluid scenes can collide with complex containers cheaply (CUDA only; meshes without an SDF and CPU runs are unchanged)
...
- Add a render-only diffuse foam/spray layer to `SolverXPBD` fluids via `max_diffuse_particles` and the `diffuse_*` parameters: Flex-style spawning from a trapped-air/wave-crest potential, fluid advection with drag and buoyancy, and optional shape collision; the resulting `diffuse_positions` / `diffuse_velocities` feed `Viewer.log_fluid_diffuse()` and whitewater is enabled in the XPBD dam-break and cup-transfer examples
...
- Add `fluid_xpbd_dam_break` example: a water column collapses across an unbounded ground plane, splashes against a pillar, and carries a floating box, demonstrating two-way coupled XPBD fluids
- Add `fluid_xpbd_cereal_bowl` example: torus-shaped cereal pieces float in a dynamic bowl of XPBD milk rendered as an opaque scattering liquid; drag the bowl or individual cereal pieces with mouse picking
- Add XPBD counterparts for the remaining fluid examples: `fluid_xpbd_interactive_tank` (boxes float or sink purely through the unified XPBD solve, with no hand-tuned buoyancy or drag forces), `fluid_xpbd_wave_pool` (a kinematic paddle drives waves that break on a sloped beach, with low-density spheres, boxes, and capsules that bob on the surface), and `fluid_xpbd_cup_transfer` (an IK-driven Franka arm carries and spills a cup of XPBD water)
...
- Tune XPBD fluid examples with `--particle-count`, deriving particle spacing, radius, mass, and grid dimensions from the requested count.
- Refine the XPBD wave-pool and cup example timesteps and density relaxation to suppress particle-scale pressure jitter while retaining fluid volume.
...
- Render the `mpm_viscous` example as a creamy vanilla-sauce liquid surface in `ViewerGL`; pass `--render-mode particles` to restore the previous raw-particle rendering
- Keep `ViewerGL` fluid vertex packing and diffuse-particle compaction and sorting on the GPU, and use separable bilateral depth smoothing to reduce fluid-rendering stalls.
...
- Skip particle compaction and GPU uploads when both particle and fluid-state rendering are hidden.
- Fix `SolverXPBD` position-based fluid over-compressing under its own weight, so a deep column of fine particles collapsed into a dense slug at the bottom of its container instead of filling it. The density correction now uses the standard summed PBF position update (Macklin & Müller 2013) instead of an over-damped per-neighbor average that scaled incompressibility down by roughly the neighbor count.
- Fix XPBD fluid density corrections cancelling shape-contact corrections and pushing particles through container walls. Fluid contacts are re-projected after particle constraints, with fixed boundaries receiving final precedence over opposing dynamic contacts, which also lets contained fluids settle without relying on viscosity.
- Fix `SolverXPBD` position-based fluid particles fusing into stuck "super particles" when compressed: the density constraint's gradient direction is undefined at zero separation and its correction is diluted by neighbor averaging, so near-coincident particles could not separate. An un-averaged minimum-separation repulsion now drives them apart.
- Fix `SolverXPBD` propagating a non-finite particle position correction through the neighbor grid (which froze the whole solve): such a particle is now reset to its pre-step position at rest, containing the failure.
- Fix `SolverXPBD` dynamic-body velocity caps applying only after pose integration, so a picking-force spike cannot cross a thin collider before the constraint solve.
- Fix XPBD fluid pressure contacts discarding the equal-and-opposite reaction on dynamic shapes after density projection, so rigid bodies float or sink according to density while conserving momentum.
...
- Fix XPBD fluid graph capture initialization and reuse the simulation hash grid for render-neighbor queries so interleaved rendering cannot invalidate captured grouped-grid state or change simulation results.
- Fix the rectangular XPBD fluid tank walls stopping half a wall thickness short at their outer corners.
- Fix particle soft-contact generation launching over non-particle shapes and undersizing its replay buffer when `soft_contact_max` is capped; candidate work and replay memory now include only particle-collidable shapes.
- Fix XPBD fluid rendering and diffuse emission including active non-fluid particles, full diffuse pools overwriting live particles, and diffuse shape friction varying with simulation frequency.
- Fix `fluid_xpbd_cereal_bowl` cereal overlap, sinking, jitter, tunneling, and sticky floor motion with torus-SDF fluid contact, compound-torus rigid proxies, tangent surface packing, two-way XPBD fluid coupling, low-friction ground contact, tighter body-frame validation, and a neutral opaque-milk material.
- Fix XPBD fluid examples advancing simulation time without launching the newly captured CUDA graph on the first frame or after recapture.
