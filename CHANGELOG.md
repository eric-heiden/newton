...
- Add `SolverSPH`, an experimental Warp-accelerated particle-fluid solver with PBF density projection, solver-side render smoothing/anisotropy, and Flex-style diffuse spray/foam particles spawned from a trapped-air/wave-crest potential.
- Add Flex-style screen-space fluid rendering to `ViewerGL` via `Viewer.log_fluid()` and `Viewer.log_fluid_diffuse()`: anisotropic quadric ellipsoid splatting, adaptive 2D bilateral depth smoothing, a composite pass with screen-space refraction/reflection, Fresnel, shadow-mapped wrap lighting and tight speculars, fluid splats in the scene shadow map, and velocity-stretched diffuse foam billboards. Ported from the NVIDIA Flex demo renderer.
- Add `fluid_sph_cup_transfer` example: a Franka arm driven by IK endlessly picks and places a transparent cup of SPH water between two spots, with a GUI speed slider; cranking the speed makes the water slosh and spill.
- Add three fluid examples: `fluid_sph_dam_break`, `fluid_sph_interactive_tank` (pickable boxes that float or sink according to per-box density via displaced-volume buoyancy with locally measured water heights), and `fluid_sph_wave_pool` (a kinematic paddle drives waves that break on a sloped beach and shed whitewater foam).
...
- Add position-based fluid (PBF) support to `SolverXPBD`: particles flagged with the new `ParticleFlags.FLUID` generate SPH density constraints solved alongside all other XPBD constraints, with controls for rest spacing, cohesion (splash coagulation), XSPH viscosity, and vorticity confinement, plus `SolverXPBD.update_render_particles()` for Flex-style smoothed/anisotropic fluid surface rendering
