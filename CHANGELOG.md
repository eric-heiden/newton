...
- Add `SolverSPH`, an experimental Warp-accelerated particle-fluid solver, and add ViewerGL screen-space fluid rendering with translucent smoothed water surfaces, tunable blur radius, tropical optical-depth color absorption, stronger env-map reflection/refraction and env transmission, scene-depth floor caustics, depth cueing, ripple/caustic distortion, and screen-space foam for particle fluids. Add a dam-break example plus an interactive tank example with suspended rigid boxes, picking forces, SPH-to-box coupling, and live GUI sliders for water shader and interaction settings.
