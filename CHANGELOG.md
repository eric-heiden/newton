...
- Add `SolverStateFlags` and `SolverMuJoCo.reset(state, world_mask=None, flags=None)` so MuJoCo solver resets update the input state in place, clear per-world solver state (including `qacc_warmstart`), and optionally restrict the reset to a subset of worlds and state attributes
