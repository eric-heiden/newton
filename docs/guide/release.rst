...
.. list-table::
   ...
   * - ☐
     - On ``release-X.Y``, preview and audit the pending fragments, then run the
       Towncrier build commands in ``changelog.d/README.md`` with the GA
       version and date.  Delete ``.skip`` fragments and merge the
       changelog-only pull request with the ``release-management`` label before
       preparing the final version.  For the first Towncrier release, also
       merge duplicate category headings while retaining every legacy
       ``[Unreleased]`` entry.
   * - ☐
...
.. list-table::
   ...
   * - ☐
     - Cherry-pick the release's Towncrier build commit onto a changelog-only
       branch from ``main`` and merge it in a pull request with the
       ``release-management`` label.  Confirm that post-cut fragments remain
       pending under ``changelog.d/``.
   * - ☐
