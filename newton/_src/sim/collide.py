...
from ..geometry.soft_contacts_sdf import create_soft_contacts_sdf
...
class CollisionPipeline:
    ...
    def __init__(
        ...
        *,
        ...
        # Compact particle-collidable shapes once. Soft-contact generation used
        # to launch over every particle x every model shape, including visual and
        # rigid-only proxy shapes; its replay IDs then indexed beyond the capped
        # output buffer. Compact regular and texture-SDF candidates keep both the
        # launch and replay storage proportional to work that can produce contact.
        self._soft_shape_sdf_index = model._shape_sdf_index
        if self._soft_shape_sdf_index is None:
            self._soft_shape_sdf_index = wp.full(shape_count, -1, dtype=wp.int32, device=device)
        flags_np = model.shape_flags.numpy() if shape_count > 0 else np.empty(0, dtype=np.int32)
        sdf_idx_np = (
            model._shape_sdf_index.numpy()
            if model._shape_sdf_index is not None
            else np.full(shape_count, -1, dtype=np.int32)
        )
        regular_shapes = [
            i
            for i in range(shape_count)
            if (int(flags_np[i]) & int(ShapeFlags.COLLIDE_PARTICLES)) != 0 and sdf_idx_np[i] < 0
        ]
        self._soft_shape_indices = wp.array(regular_shapes, dtype=wp.int32, device=device) if regular_shapes else None
        self._num_soft_shapes = len(regular_shapes)
        self._soft_sdf_shape_indices = None
        self._num_soft_sdf_shapes = 0
        if model._shape_sdf_index is not None and wp.get_device(device).is_cuda and shape_count > 0:
            sdf_shapes = [
                i
                for i in range(shape_count)
                if sdf_idx_np[i] >= 0 and (int(flags_np[i]) & int(ShapeFlags.COLLIDE_PARTICLES)) != 0
            ]
            if sdf_shapes:
                self._soft_sdf_shape_indices = wp.array(sdf_shapes, dtype=wp.int32, device=device)
                self._num_soft_sdf_shapes = len(sdf_shapes)
        self._soft_sdf_replay_tid_offset = particle_count * self._num_soft_shapes
        self._soft_contact_candidate_max = particle_count * (self._num_soft_shapes + self._num_soft_sdf_shapes)
        self.requires_grad = requires_grad
    ...
    def contacts(self) -> Contacts:
        ...
        contacts = Contacts(
            ...
            soft_contact_candidate_max=self._soft_contact_candidate_max,
        )
    ...
    def collide(
        ...
        *,
        ...
        if state.particle_q and self._num_soft_shapes > 0:
            wp.launch(
                kernel=create_soft_contacts,
                dim=particle_count * self._num_soft_shapes,
                inputs=[
                    state.particle_q,
                    ...
                    model.shape_source_ptr,
                    model.shape_world,
                    self._soft_shape_indices,
                    self._num_soft_shapes,
                    soft_contact_margin,
                    self.soft_contact_max,
                    model.shape_flags,
                    self._soft_shape_sdf_index,
        ...
        # Cheap SDF-sampled soft contacts for shapes carrying a texture SDF.
        if state.particle_q and self._num_soft_sdf_shapes > 0:
            wp.launch(
                kernel=create_soft_contacts_sdf,
                dim=particle_count * self._num_soft_sdf_shapes,
                inputs=[
                    state.particle_q,
                    model.particle_radius,
                    model.particle_flags,
                    ...
                    self._soft_sdf_shape_indices,
                    self._num_soft_sdf_shapes,
                    self._soft_sdf_replay_tid_offset,
                    soft_contact_margin,
                    self.soft_contact_max,
                ],
                outputs=[
